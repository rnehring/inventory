...
function getPart(event) {
    ...
    axios.post('/inventory-precount-search', {
        ...
        .then(function (response) {
            ...
            parts.forEach((part) => {
                ...
                let bin_verified = makeCell(`<input type='checkbox' class='mx-auto block px-2' id='bin_verified${part['id']}' ${part['bin_verified'] === 1 ? 'checked />' : '/>'}`);
                ...
                save_button.addEventListener('click', function(event){
                    let bin_verified = document.getElementById('bin_verified' + part['id']);
                    if (bin_verified.checked) {
                        let verified = 1;
                    } else {
                        let verified = 0;
                    }
                    axios.post('/update-precount', {
                        count: document.getElementById('count'+part['id']).value,
                        part: part['id'],
                        bin_verified: verified
                    })
                        .then(function (response) {
